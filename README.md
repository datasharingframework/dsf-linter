...
| **StructureDefinition 1.0.0**  `FhirStructureDefinitionValidator` <br>(**NEW in v1.3.0**) |  `differential`, `url`                           | \* Must contain `meta.tag` .<br>\* `version` and `date` fields require `#{version}` / `#{date}` placeholders.<br>\* No `snapshot` allowed; all element `@id`s unique.<br>\* **Slice cardinality**: min/max constraints are verified for each slice using rules from FHIR §5.1.0.14 ([build.fhir.org](https://build.fhir.org/profiling.html?utm_source=chatgpt.com)). | Placeholder enforcement; read‑access tag                                                                 |
