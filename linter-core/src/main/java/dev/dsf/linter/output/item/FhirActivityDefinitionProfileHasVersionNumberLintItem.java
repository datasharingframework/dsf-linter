...
/**
 * Validation item indicating that the ActivityDefinition profile contains a version number,
...
public class FhirActivityDefinitionProfileHasVersionNumberLintItem extends FhirElementLintItem {
    /**
     * Constructs a new validation item for a profile with version number using a default description.
     *
    ...
    /**
     * Constructs a new validation item for a profile with version number using a custom description.
     *
     * @param resourceFile  the file where the FHIR resource was loaded from
     * @param fhirReference a canonical URL or local reference that identifies the resource
