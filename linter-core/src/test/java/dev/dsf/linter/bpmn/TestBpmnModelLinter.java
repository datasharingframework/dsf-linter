...
/**
 * Test suite for the {@link BpmnModelLinter}.
 * <p>
 * This class contains unit tests to verify the correct behavior of the BPMN model linter.
 ...
 * Each test method builds a specific BPMN model and asserts that the linter detects the expected issues.
 * </p>
 * <p>
 ...
 * </p>
 */
class TestBpmnModelLinter {

    private BpmnModelLinter linter;
    private File dummyBpmnFile; // Reference file passed into the linter
    private String dummyProcessId;

    /**
     * Setup method executed before each test.
     * <p>
     * This method initializes the {@link BpmnModelLinter} with a dummy project root used to simulate.
     * It also sets up a dummy BPMN file reference and a process ID that will be used in subsequent tests.
     * </p>
     */
    @BeforeEach
    void setup() {
        // Create a new linter with an explicit project root for class loading
        linter = new BpmnModelLinter(new File("."));
     ...
     * It asserts that the linter returns at least one lint issue for this misconfiguration.
     * </p>
     */
    @Test
    @DisplayName("1) ServiceTask: No name, no implementation => expect lint issues")
    void testServiceTaskNoNameNoImplementation() {
        ...
        List<BpmnElementLintItem> issues = linter.lintModel(model, dummyBpmnFile);

        // We expect at least one issue (e.g., "no name", "no implementation class")
        assertFalse(issues.isEmpty(), "Expected lint issues for a ServiceTask with no name / no impl");
    }
     ...
     * The test expects the linter to return at least one lint issue for the misconfiguration.
     * </p>
     */
    @Test
     ...
     * The linter is expected to detect this misconfiguration and return a lint issue.
     * </p>
     */
    @Test
     ...
     * all of which do not have names. The linter is expected to generate warnings regarding the unnamed flows.
     * </p>
     */
    @Test
     ...
     * The linter is expected to detect the missing asyncBefore configuration for the multi-instance SubProcess.
     * </p>
     */
    @Test
    @DisplayName("5) SubProcess with multi-instance but not asyncBefore => expect a lint item")
    void testMultiInstanceSubProcessNotAsyncBefore() {
        ...
        // lint the model and check for multi-instance misconfiguration
        List<BpmnElementLintItem> issues = linter.lintModel(model, dummyBpmnFile);

        // Typically, we expect an issue regarding the multi-instance SubProcess not having asyncBefore enabled
        assertFalse(issues.isEmpty(),
                "Should detect that multi-instance SubProcess is not asyncBefore if your linter enforces that");
    }
     ...
     * The linter is expected to return at least one lint issue for the missing name.
     * </p>
     */
    @Test
    @DisplayName("6) UserTask with no name => expect lint issue")
     ...
     * The linter is expected to detect this configuration error and return a lint issue.
     * </p>
     */
    @Test
